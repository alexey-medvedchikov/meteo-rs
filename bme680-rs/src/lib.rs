...
const POLL_PERIOD_MS: u8 = 10;
const CHIP_ID: u8 = 0x61;
const SOFT_RESET_CMD: u8 = 0xb6;

pub enum Error<R, W> {
    BusError(i2cutil::Error<R, W>),
    BoundaryCheckFailure(&'static str),
    Settings(settings::Error),
    DeviceNotFound,
}

impl<R, W> From<i2cutil::Error<R, W>> for Error<R, W> {
    fn from(value: i2cutil::Error<R, W>) -> Self {
        Self::BusError(value)
    }
}

impl<R, W> From<settings::Error> for Error<R, W> {
...
pub struct I2CAddress(i2c::SevenBitAddress);

impl I2CAddress {
    pub const PRIMARY: Self = Self(0x76);
    pub const SECONDARY: Self = Self(0x77);
...
impl From<I2CAddress> for i2c::SevenBitAddress {
...
impl<'a, I2C, R, W> Bme680<'a, I2C>
where
    I2C: i2c::Read<Error = R> + i2c::Write<Error = W>,
{
    pub fn new(i2c: &'a mut I2C, address: I2CAddress) -> Result<Self, Error<R, W>> {
        let chip_id = i2cutil::read_byte(i2c, address, CHIP_ID_ADDR)?;
    ...
    pub fn soft_reset<D>(&mut self, delay: &mut D) -> Result<(), i2cutil::Error<R, W>>
    where
        D: DelayMs<u8>,
    {
        i2cutil::write(self.i2c, self.address, &[SOFT_RESET_ADDR, SOFT_RESET_CMD])?;
    ...
    ) -> Result<(), i2cutil::Error<R, W>> {
        let mut data = i2cutil::read_byte(self.i2c, self.address, CONF_ODR_FILTER_ADDR)?;
        data = (data & !0x1c) | (((settings.iir_filter as u8) << 2) & 0x1c);
        i2cutil::write(self.i2c, self.address, &[CONF_ODR_FILTER_ADDR, data])
    }

    fn set_heater_control(&mut self, settings: &GasSettings) -> Result<(), i2cutil::Error<R, W>> {
        let mut data = i2cutil::read_byte(self.i2c, self.address, CONF_HEAT_CTRL_ADDR)?;
        data = (data & !0x8) | (settings.heater_control as u8 & 0x8);
        i2cutil::write(self.i2c, self.address, &[CONF_HEAT_CTRL_ADDR, data])
    }
    ...
    fn set_temperature_and_humidity_oversampling(
        ...
        let mut data = i2cutil::read_byte(self.i2c, self.address, CONF_POWER_MODE_ADDR)?;
        data = (data & !0xe0) | (((settings.temperature_oversampling as u8) << 5) & 0xe0);
        data = (data & !0x1c) | (((settings.pressure_oversampling as u8) << 2) & 0x1c);
        i2cutil::write(self.i2c, self.address, &[CONF_POWER_MODE_ADDR, data])
    }
    ...
    fn set_humidity_oversampling(
        ...
        let mut data = i2cutil::read_byte(self.i2c, self.address, CONF_OS_H_ADDR)?;
        data = (data & !0x7) | (settings.humidity_oversampling as u8 & 0x7);
        i2cutil::write(self.i2c, self.address, &[CONF_OS_H_ADDR, data])
    }

    fn set_run_gas_and_nb_conversion(
        &mut self,
        settings: &GasSettings,
    ) -> Result<(), i2cutil::Error<R, W>> {
        let mut data = i2cutil::read_byte(self.i2c, self.address, CONF_ODR_RUN_GAS_NBC_ADDR)?;
        data = (data & !0x10) | (((settings.enable_gas_measurement as u8) << 4) & 0x10);
        data = (data & !0xf) | (settings.nb_conversion as u8 & 0xf);
        i2cutil::write(self.i2c, self.address, &[CONF_ODR_RUN_GAS_NBC_ADDR, data])
    }

    fn set_gas(&mut self, settings: GasSettings) -> Result<(), i2cutil::Error<R, W>> {
        let heater_resistance = calc::heater_temperature(
            &self.calibration_data,
            settings.ambient_temperature,
            settings.heater_temperature,
        );
        i2cutil::write(self.i2c, self.address, &[RES_HEAT0_ADDR, heater_resistance])?;
        let heater_duration = calc::heater_duration(settings.heater_duration_ms);
        i2cutil::write(self.i2c, self.address, &[GAS_WAIT0_ADDR, heater_duration])
    }

    pub fn set_settings<D>(&mut self, delay: &mut D, settings: Settings) -> Result<(), Error<R, W>>
    where
        D: DelayMs<u8>,
    {
        self.set_power_mode(delay, PowerMode::Forced)?;

        self.set_gas(settings.gas)?;
    ...
    pub fn get_settings(&mut self) -> Result<Settings, Error<R, W>> {
        const REG_BUFFER_LENGTH: usize = 6;
        let mut buf = [0; REG_BUFFER_LENGTH];
        let mut settings: Settings = Default::default();

        i2cutil::read_bytes(self.i2c, self.address, CONF_ADDR_START, &mut buf)?;
    ...
    pub fn get_power_mode(&mut self) -> Result<PowerMode, Error<R, W>> {
        let regs = i2cutil::read_byte(self.i2c, self.address, CONF_POWER_MODE_ADDR)?;
        Ok(PowerMode::try_from(regs & MODE_MASK)?)
    }

    const OS_TO_MEAS_CYCLES: [u8; 6] = [0, 1, 2, 4, 8, 16];

    pub fn get_profile_duration(&self, settings: &Settings) -> Result<u32, Error<R, W>> {
        ...
        let temp_duration = ((cycles as u32) * 1963 + 477 * 4 + 477 * 5 + 500) / 1000 + 1;
    ...
    fn get_calibration_data(
        i2c: &mut I2C,
        address: I2CAddress,
    ) -> Result<CalibrationData, Error<R, W>> {
        ...
        calibration_data.res_heat_range =
            (i2cutil::read_byte(i2c, address, ADDR_RES_HEAT_RANGE_ADDR)? & 0x30) / 16;

        calibration_data.res_heat_val =
            i2cutil::read_byte(i2c, address, ADDR_RES_HEAT_VAL_ADDR)? as i8;

        calibration_data.range_sw_err =
            (i2cutil::read_byte(i2c, address, ADDR_RANGE_SW_ERR_ADDR)? & RSERROR_MASK) / 16;

        Ok(calibration_data)
    }

    fn get_gas_config(&mut self) -> Result<GasSettings, Error<R, W>> {
        let heater_temperature =
            i2cutil::read_byte(self.i2c, self.address, ADDR_SENS_CONF_START)? as u16;
        let heater_duration_ms =
            i2cutil::read_byte(self.i2c, self.address, ADDR_GAS_CONF_START)? as u32;
    ...
    pub fn get_readings(&mut self) -> Result<(Readings, ReadingsCondition), Error<R, W>> {
        let mut buf = [0u8; 15];
        i2cutil::read_bytes(self.i2c, self.address, FIELD0_ADDR, &mut buf)?;
